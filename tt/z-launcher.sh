#!/bin/bash
# z-launcher.sh — paneboard's tabtarget trampoline.
#
...
# tabtarget. BURD_LAUNCHER flows downstream unchanged (it is the regime variable
# required by burd_regime and allowlisted by bud_dispatch).
#
# No-log behavior is NOT a launcher selection: it rides the BURD_NO_LOG env var
# the tabtarget exports ahead of dispatch (bul_launcher skips the BURS station
# load under it).
...
# paneboard keeps its moorings/config dir at .buk. The shared kit (bul_launcher,
# bubc) consumes BURD_CONFIG_DIR exported below rather than hardcoding a name —
# so one kit serves every consumer.
z_moorings_dir=".buk"
