...
zjjw_emit_claudemd_section() {
  {
    ...
    echo ""
    echo "**BUK Basics:**"
    echo "- **TabTarget**: Shell scripts in \`tt/\` providing tab-completable commands; filename encodes routing info"
    echo "- **Colophon**: First token (e.g., \`jjw-hs\`) - the routing identifier"
    echo "- **Frontispiece**: Second token (e.g., \`HeatSaddle\`) - human-readable description"
    echo "- **Formulary**: Routing component (workbenches, testbenches) that maps colophons to implementations"
    echo "- For detailed BUK patterns: \`Tools/buk/README.md\`"
    echo ""
